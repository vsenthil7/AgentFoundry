...
|--------|-------|--------|---------------------------|
| S89 | Login-persistence fix + durable-auth deploy + demo click-to-fill | ⬜ | Make the live deploy durable: server selects AF_DATA/AF_PG so AuthService persists across restart; deploy override mounts AF_DATA volume; document the fixed deploy. Web: "Use demo account" button on login auto-fills owner@acme.test/demo-password-123 and submits; register stays reachable. Reproduce + add a regression test that registering then re-logging-in (same process and after a store reload) succeeds. Fix the "email and password required" mis-fire. DoD: register→logout→login round-trips on a durable store, proven by test + verified live. |
| S90 | AuthService: profile self-service + password change (backend) | ⬜ | Extend AuthService/IdentityStore: `updateProfile(userId,{displayName?,email?})` (email uniqueness re-checked, persisted), `changePassword(userId, current, next)` (verifies current via constant-time compare, re-hashes, persists, optionally revokes other sessions). Add `displayName` to User. Endpoints: PATCH /auth/profile, POST /auth/password. Negative paths: wrong current password 401, duplicate email 409, weak new password 400. 100% coverage. |
| S91 | AuthService: tenant-admin user management (backend) | ⬜ | Admin-only, tenant-scoped: `createUser` (admin sets email+role+temp password), `setUserRoles`, `deactivateUser`/`reactivateUser` (deactivated users can't log in — login checks active flag), `resetUserPassword` (admin issues a temp password). Add `active` to User. Endpoints under /admin/users (POST create, PATCH :id roles, POST :id/deactivate, POST :id/reactivate, POST :id/reset-password). RBAC: admin:manage_users; can't touch other tenants; can't deactivate the last admin. 100% coverage. |
| S92 | Superadmin: cross-tenant role + platform console (backend) | ⬜ | New `superadmin` role + `admin:platform` permission that bypasses tenant scoping for read + tenant lifecycle. Endpoints: GET /platform/tenants (all tenants + user counts + status), GET /platform/tenants/:id/users, POST /platform/tenants (provision via S43 onboarding), POST /platform/tenants/:id/suspend. Superadmin is provisioned by env (AF_SUPERADMIN_EMAIL) at boot, never self-registerable. Tenant isolation for normal admins unchanged + re-tested. 100% coverage. |
| S93 | Human-in-the-loop reviewer queue (backend wiring) | ⬜ | Surface the existing S16 ReviewQueue over HTTP for reviewers/admins: GET /reviews (pending, tenant-scoped — already exists, extend), GET /reviews/:id, POST /reviews/:id/approve {reason}, POST /reviews/:id/reject {reason}. Approve/reject resolve the queue item, emit events, write the tamper-evident ledger entry, and (approve) drive the promotion gate. RBAC: reviewer or admin. Reason is required on reject. 100% coverage. |
...
| S98 | Superadmin platform console screen | ⬜ | Superadmin-only "Platform" section: tenants table (name, users, status), drill into a tenant's users, provision-tenant modal, suspend-tenant with confirm. Wired to S92. Hidden entirely for non-superadmins (nav + route guard). Component tests + Playwright. |
| S99 | Human-in-the-loop reviewer inbox screen | ⬜ | Reviewer/admin "Reviews" inbox: pending list, open an item (agent + score + context), approve / reject-with-required-reason, reflects resolution. Wired to S93. Component tests + Playwright (approve + reject-reason paths). |
| S100 | Golden Thread console redesign | ⬜ | The compose→evaluate→redteam→score→approve→export pipeline re-rendered as a guided stepper with status, results in cards/tables (not dense metric rows), score/coverage as readable visualizations. Keep data-testids so golden-thread.spec passes; lift App.tsx branch coverage where redesign removes dead ternaries. |
| S101 | Admin operator cockpit redesign | ⬜ | The operator tabs (users / API audit / circuit breakers / run-replay) become proper data tables with sorting, status pills, empty states, and confirm-dialog for breaker reset. Sidebar nav replaces testid-button tabs (testids retained). |
| S102 | Quota + observability dashboard | ⬜ | A tenant-health dashboard: quota usage bars vs caps (S88), error-rate + latency from metrics (S18), recent audit summary. New view + tests + Playwright. |
