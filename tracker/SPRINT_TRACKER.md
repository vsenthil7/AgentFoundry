...
| S124 | Battle Mode Arena — surface the creative hook that's already built (enterprise-grade AND creative) | ⬜ | **PLANNED — NOT BUILT.** **Reframed (15/06):** "enterprise-grade" and "creative" are NOT opposites — Figma, Photoshop, Linear are all both. AgentFoundry's creative hook **already exists in the engine**: the S3 Battle Mode red-team battery (named attacks, each mapped to OWASP/ATLAS/NIST, pass/fail per attack = "did the agent defend?", anti-weaponization classifier). The gap is not creativity — it's that this resolves to a *results table*, not an *experience a judge watches*. **Planned:** turn the existing `ATTACK_BATTERY`/`AttackResult` data into a live **arena**: attacks land one-by-one, the agent visibly defends or leaks, framework IDs light up as each lands, the deterministic verdict is the climax — the 90-second creative spine a Creative-Apps judge feels in the first 30 seconds, surfaced as the headline view (not a tab behind a login). Built on the design system (S94 primitives) so it stays credible/enterprise, not a toy; deterministic gate UNTOUCHED (engine still scores). The enterprise rigor (real frameworks, real scoring, audit trail) is what makes the creative concept *credible* rather than a gimmick — the two reinforce each other. **DoD:** an arena view driven by the real redteam engine; live attack-by-attack sequence; framework provenance visible; deterministic verdict as climax; works on web + mobile; reachable as a headline experience; 100% coverage on new web modules + tsc; Playwright E2E for the arena flow; demo-offline still green; tracker/traceability/KNOWN_GAPS; clean commit. **Sequencing:** S123 (eligibility) first — a great arena on an ineligible entry wins nothing — then S124. Both gated on the deadline answer. |
