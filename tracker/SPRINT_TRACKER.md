...
| S125 | Arena narration + attack/defense storytelling layer | ⬜ | **PLANNED — NOT BUILT.** Make each arena round *legible to a non-expert judge*: a pure `arena/narration.ts` that, per attack class (prompt_injection / pii_exfiltration / jailbreak / tool_abuse / data_poisoning), produces a plain-language "what the attacker tried" line, "how the agent responded" line, and "why this matters" framing tied to the framework IDs — deterministic strings, no LLM, fully testable. `BattleArena` renders these as a narrated play-by-play so the 90-second demo explains itself without a presenter. Includes a per-class severity ordering so the most dramatic rounds are paced well. **DoD:** `narration.ts` pure, exhaustive over all `AttackClass` values (compile-time `satisfies Record<AttackClass,…>`), 100% all four; arena renders narration; Playwright assertion; demo-offline green. |
| S126 | Agent Loadout — compose-your-defender creative interaction | ⬜ | **PLANNED — NOT BUILT.** Turn the static seed agent into a **playable loadout** before battle: a `arena/loadout.ts` pure reducer + `arena/Loadout.tsx` letting the user toggle defensive capabilities that map to REAL design nodes (guardrail on/off, grounding on/off, prompt-hardening) and see the predicted effect, then run the arena with that exact design. This is the creative "build then watch it fight" loop the product was always meant to have — and it's honest because the toggles drive the real `runBattle` (guardrail-off genuinely leaks, guardrail-on genuinely defends, per the deterministic engine). **DoD:** `loadout.ts` pure reducer 100% all four; `Loadout.tsx` 100% all four; chosen loadout feeds the real engine; defend-rate changes are real not cosmetic; Playwright E2E (toggle guardrail off → arena shows breaches); demo-offline green. |
| S127 | Arena scorecard share-card + replayable verdict | ⬜ | **PLANNED — NOT BUILT.** The shareable creative payoff: a pure `arena/scorecard.ts` that composes the battle outcome (defend-rate, per-class results, framework coverage, certification tier from the existing S9 cert engine) into a deterministic **share-card** model, rendered as an SVG-style results card (`arena/ScoreCard.tsx`) a judge/viewer can screenshot — "Acme Support Bot defended 4/4 attacks · OWASP+ATLAS+NIST covered · GOLD". Deterministic, offline, no external calls. Adds a "replay this battle" affordance (re-runs the same deterministic battle). **DoD:** `scorecard.ts` pure 100% all four; `ScoreCard.tsx` 100% all four; tier pulled from the real certification engine; Playwright assertion; demo-offline green. |
| S128 | Creative arc audit + arena as the demo Golden Thread headline | ⬜ | **PLANNED — NOT BUILT.** Cross-cutting close of the creative arc: make the Battle Mode Arena the **first thing** in the offline demo narrative + the landing experience post-login (without breaking the existing console contract), refresh `demo.ts`/`make demo-offline` so the 90-second spine is Loadout → Arena → ScoreCard → (governance depth underneath), update USER_GUIDE + a new `docs/CREATIVE_NARRATIVE.md` describing the judge-facing story, and re-verify full-suite green (backend + web at 100%, tsc, build, demo-offline). **DoD:** arena is the headline; demo narrative leads with it; docs written; everything still green; clean commit; honest note if anything deferred. |
...
- **Web component (jsdom):** 322 tests passing · every screen module incl. auth/AuthGate.tsx (S120 SSO sign-in) + compliance/ComplianceScreen.tsx + status/StatusHistoryScreen.tsx + governance/DataGovernanceScreen.tsx + marketplace/MarketplaceScreen.tsx + secrets/SecretsScreen.tsx at **100%** all four metrics; App.tsx 100% stmts/funcs/lines (branch **87.8%** after S118 — the residual 10 arms are unreachable-by-construction, see KNOWN_GAPS §2)
- **Web build:** production build succeeds (79 modules)
