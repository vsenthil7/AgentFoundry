...
| S106 | Secrets & connectors read screen | ⬜ | New `secrets/SecretsScreen.tsx` (admin) wired to the S17 vault over HTTP: lists per-tenant secret *handles* (masked head…tail, never plaintext) + connector references (MCP/OpenAPI/A2A) in design-system Tables, with last-rotated metadata. Read-first (create/rotate deferred to a later sprint, noted). New authClient.listSecrets/listConnectors + types; requires a backend GET /secrets + /connectors surface — add the read endpoints if absent (admin-gated, tenant-scoped, masked). New `secrets/secrets.css`. Nav item in AuthedApp (admin). SecretsScreen + authClient at 100%. |
| S107 | Billing & invoices read screen | ⬜ | New `billing/BillingScreen.tsx` (admin) wired to the S34/S37 invoice surfaces: current-period usage → priced line items, invoice history table (period / subtotal / total / currency), and lifetime summary, all from real backend reads. New authClient.getCurrentInvoice/listInvoices + types; add GET /billing/current + /billing/history if absent (admin-gated, tenant-scoped). Money always in integer minor units, formatted at the edge. New `billing/billing.css`. Nav item in AuthedApp (admin). BillingScreen + authClient at 100%. |
| S108 | Keyboard-accessible nav + active-route a11y polish | ⬜ | AppShell nav becomes fully keyboard operable: arrow-key roving tabindex across nav items, Enter/Space activate, `aria-current=page` already present, focus-visible ring honoured, and the mobile drawer traps focus + closes on Escape. Playwright: tab to nav, arrow down, Enter → view switches; Escape closes the drawer on web-mobile. No behaviour regressions to existing testids. |
| S109 | Final buyer-readiness audit + submission snapshot | ⬜ | Cross-cutting sweep: re-verify full system green (backend + web + both builds + tsc), confirm every nav destination reachable per role, refresh KNOWN_GAPS.md (close the "satellite screens not unified" + nav-integration gaps now that S105 lands them), refresh USER_GUIDE.md navigation section, re-validate the compose port story, and record the final test counts. Definition: a reviewer can log in, see one coherent role-aware product, and reach every screen from the sidebar. |
