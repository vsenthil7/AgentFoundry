...
| S111 | Compliance & audit export screen | ⬜ | The `GET /compliance/pack`, `GET /compliance/history`, and `GET /audit/export` routes exist (S59/S76/S53) but have **no UI** — a high-impact reviewer moment for a governance product. Web: authClient `getCompliancePack`/`getComplianceHistory`/`getAuditExport` + types; new `compliance/ComplianceScreen.tsx` (admin) — renders the compliance-pack markdown in a readable panel, a snapshot-history Table with the latest posture diff, and a "download signed audit export" action (verified-signature Badge); loading/error/empty states. Nav item **Compliance** in AuthedApp (admin) + route + guard + vitest include. New web code at 100% all four metrics. |
| S112 | Status history / trend screen | ⬜ | `GET /status` + `GET /status/history` exist (S47/S75) but only the live snapshot is shown (HealthDashboard); the **trend over time has no UI**. Web: authClient `getStatusHistory` + `StatusHistory`/`StatusTrend` types; new `status/StatusHistoryScreen.tsx` (admin + ops) — trend Badge (improving/stable/worsening), state-fraction bars (healthy/degraded/down), and a samples Table; loading/error/empty states. Nav item **Trend** in AuthedApp (admin/ops) + route + guard + vitest include. New web code at 100% all four metrics. |
| S113 | Data residency & retention screen (+ route) | ⬜ | The S19 DataGovernance engine has **neither route nor screen**. Add an admin-gated, tenant-scoped `GET /governance/data` route serving the per-tenant residency report (allowed regions, record counts by region) + retention policies (data class → retention days) from an injected DataGovernance provider (404/403 guards). Web: authClient `getDataGovernance` + types; new `governance/DataGovernanceScreen.tsx` (admin) — residency Table (region / record count / allowed) + retention Table (class / days / indefinite Badge); loading/error/empty states. Nav item **Data** in AuthedApp (admin) + route + guard + vitest include. Backend + web new code at 100% all four metrics. |
| S114 | Marketplace browse screen (+ route) | ⬜ | The S10 Marketplace engine has **neither route nor screen**. Add a `GET /marketplace` route serving the tier-gated catalog (packs filtered to the caller's max certification tier, kind/publisher filters as query params) from an injected Marketplace provider (404 when unconfigured; available to any authed user). Web: authClient `browseMarketplace` + `MarketplacePack` types; new `marketplace/MarketplaceScreen.tsx` (all roles) — catalog Table/cards (name / kind Badge / publisher / tier Badge / install count), kind filter; loading/error/empty states. Nav item **Marketplace** in AuthedApp (all roles) + route + vitest include. Backend + web new code at 100% all four metrics. |
| S115 | Secrets write-path (create / rotate) | ⬜ | Promote the S106 read-only Secrets screen to full management. Backend: admin-gated, tenant-scoped `POST /secrets` (create: id+name+value), `POST /secrets/:id/rotate` (new value), `DELETE /secrets/:id` on the S17 vault (masked response, never echoes plaintext; 404/403/409-duplicate guards). Web: authClient `createSecret`/`rotateSecret`/`deleteSecret`; extend `SecretsScreen.tsx` with an "Add secret" modal (value shown only at entry, never re-fetched), per-row Rotate + Delete with confirm. Backend + web new code at 100% all four metrics. Closes the S106 documented follow-up. |
| S116 | Phase E audit + submission snapshot | ⬜ | Cross-cutting verification: full-system re-verify green (backend + web + both builds + tsc), confirm every Phase E screen reachable per role from the sidebar, refresh KNOWN_GAPS.md (record which dormant capabilities are now surfaced and which intentionally remain engine-only — e.g. replication/backup/scheduler are operator-infra, not end-user screens), refresh USER_GUIDE.md with the new screens + roles table. Definition: the gap between "built backend" and "visible in the product" is closed or explicitly, honestly documented. |

## Test Results (verified, this environment)
- **Backend engine:** 1120 tests passing · **100%** lines / branches / functions / statements (S110 added the SLA read route; was 1116)
- **Web component (jsdom):** 255 tests passing · every screen module incl. sla/SlaScreen.tsx at **100%** all four metrics; App.tsx 100% stmts/funcs/lines (branch 77% — defensive UI ternaries, see KNOWN_GAPS §2)
- **Web build:** production build succeeds (71 modules)
- **demo-offline:** Golden Thread walks 79 steps with zero network
- **Playwright E2E:** suite authored for web-desktop@1280 + web-mobile@Pixel7 — auth shell (login/register/admin/logout/demo + bad-credentials negative + S108 keyboard nav), the Golden Thread driven through the login gate, and the S103 responsive layout checks. **42 tests across 3 files** (`auth.spec.ts`, `golden-thread.spec.ts`, `responsive.spec.ts`); runs green on a normal-network machine (browser CDN blocked in this build env — KNOWN_GAPS §1).
