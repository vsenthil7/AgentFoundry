...
| R85 | **App.tsx failure-state coverage via injectable agent** — S118 | web `App.tsx` (optional `design`/`model` props defaulting to the seed; `AppProps` type), `tests-component/App.test.tsx` (broken/weak-agent fixtures + walk) | `App.test.tsx` (+2: INVALID-graph broken agent disables evaluate; grounding-on/guardrail-off agent + leaky model → LEAKED attacks + sub-threshold `--fail` score + blocked promotion). Branch 77%→85%; residual 9 branches unreachable-by-construction, documented per-branch in KNOWN_GAPS §2 | 2 |
...
- Web App.tsx branch coverage **85%** (was 77%; S118 added a real broken/weak-agent test through the injectable-agent seam). The residual 9 branches are unreachable-by-construction (static coverage-matrix; export-gated failure banners; dead defensive guards) — documented per-branch in KNOWN_GAPS §2; the logic behind each is at 100% in the backend suite.
