...
| R78 | **Compliance & audit export screen** — S111 (planned) | web `compliance/ComplianceScreen.tsx` + `compliance.css`, `auth/authClient.ts` (getCompliancePack/getComplianceHistory/getAuditExport), `AuthedApp.tsx` (Compliance nav + route), `main.tsx` + `vitest.config.ts` (routes already exist S53/S59/S76) | `ComplianceScreen.test.tsx`, `authClient.test.tsx` | planned |
