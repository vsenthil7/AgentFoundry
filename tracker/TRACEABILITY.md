...
| R79 | **Status history / trend screen** — S112 | web `status/StatusHistoryScreen.tsx` + `status.css`, `auth/authClient.ts` (getStatusHistory + StatusHistorySummary/StatusTrend types + exported pctLabel), `AuthedApp.tsx` (Trend nav + route), `main.tsx` + `vitest.config.ts` (route already exists S75) | `StatusHistoryScreen.test.tsx` (+8: pctLabel, loading, full render incl. fraction bars, empty zero-samples, API+generic error, unmount resolve+reject), `authClient.test.tsx` (+1) | 9 |
| R80 | **Data residency & retention screen (+ GET /governance/data route)** — S113 | backend `api_server.ts` (dataGovernanceProvider dep + GET /governance/data), web `governance/DataGovernanceScreen.tsx` + `governance.css`, `auth/authClient.ts` (getDataGovernance + DataGovernanceView/DataRegion types + exported residencyRows/retentionRows), `AuthedApp.tsx` (Data nav + route), `main.tsx` + `vitest.config.ts` | `auth_api.test.ts` (+4: admin reads policy+residency, viewer 403, tenant-scoped empty, 404 unconfigured), `DataGovernanceScreen.test.tsx` (+9: helpers, loading, allowed/not-allowed+indefinite render, empty, API+generic error, unmount resolve+reject), `authClient.test.tsx` (+1) | 14 |
| R81 | **Marketplace browse screen (+ GET /marketplace route)** — S114 | backend `api_server.ts` (marketplaceProvider dep + GET /marketplace, any authed user), web `marketplace/MarketplaceScreen.tsx` + `marketplace.css`, `auth/authClient.ts` (browseMarketplace + MarketplaceCatalog/MarketplacePack/PackKind/CertificationTier types + exported filterByKind), `AuthedApp.tsx` (Marketplace nav + route), `main.tsx` + `vitest.config.ts` | `auth_api.test.ts` (+4: any-user browse + installs, viewer 200 not gated, 401 no-token, 404 unconfigured), `MarketplaceScreen.test.tsx` (+8: filterByKind, loading, render, filter-click, empty-filter, API+generic error, unmount resolve+reject), `authClient.test.tsx` (+1) | 14 |
| R82 | **Secrets write-path (create/rotate/delete)** — S115 | backend `secrets.ts` (deleteSecret + SecretInUseError on the S17 vault), `api_server.ts` (POST /secrets, POST /secrets/:id/rotate, DELETE /secrets/:id; error imports), web `secrets/SecretsScreen.tsx` (add/rotate/delete UI + secrets.css), `auth/authClient.ts` (createSecret/rotateSecret/deleteSecret) | `auth_api.test.ts` (+9: create 201/400/409, rotate 200/400/404, delete 200/404/409-in-use, viewer 403, null-body 400, all-404-unconfigured, create+rotate+delete 500 rethrow), `SecretsScreen.test.tsx` (+11: add open/submit/reload, disabled-until-valid, cancel, dup 409, rotate flow+cancel, delete+reload, in-use 409, generic errors), `authClient.test.tsx` (+3) | 23 |
| R83 | **Phase E audit + submission snapshot** — S116 | `docs/KNOWN_GAPS.md` (§8 dormant-UI backlog RESOLVED with before/after table + intentionally-engine-only list; §3 points at §8), `docs/USER_GUIDE.md` (§11–15 SLA/Compliance/Trend/Data/Marketplace, §9 Secrets write-path, sidebar list, roles table) | full-system re-verify (1137 backend + 312 web = 1449 tests, both builds + tsc clean, offline demo Golden-Thread marker), nav-reachability via AuthedApp nav-navigation test | verification sprint |
| R84 | **Real JWKS/RS256 OIDC verifier (Entra-ready)** — S117 | backend `oidc.ts` (asyncVerify config + validateAsync/resolveAsync; sync path untouched), new `oidc_jwks.ts` (buildEntraVerifier on jose createRemoteJWKSet+jwtVerify, Entra-claim→TokenClaims mapping, role map, local-JWKS test seam), `package.json` (jose dependency now wired) | `oidc_jwks.test.ts` (real RS256 keypair sign+verify valid path, wrong-audience/issuer/expired/bad-signature rejected, role-map incl. unknown→viewer, missing-claims, remote-JWKS construction) + `oidc.test.ts` (+ async path: validateAsync/resolveAsync valid+all reject reasons + no-asyncVerify) | 22 |
| R85 | **App.tsx failure-state coverage via injectable agent** — S118 | web `App.tsx` (optional `design`/`model` props defaulting to the seed; `AppProps` type), `tests-component/App.test.tsx` (broken/weak + good-but-imperfect agent fixtures + walks) | `App.test.tsx` (+3: INVALID-graph broken agent disables evaluate; grounding-on/guardrail-off agent + leaky model → LEAKED attacks + sub-threshold `--fail` score + blocked promotion; good-but-imperfect agent grounds 3/4 → passes gate, exports, silver/6-7 badges, Grounded badge unearned). Branch 77%→87.8%; residual 10 arms unreachable-by-construction, documented per-branch in KNOWN_GAPS §2 | 3 |
| R86 | **Deploy script + docs: flat path, live port 8096, collision guard** — S119 | `deploy/deploy-vultr.sh` + `deploy/deploy-vultr.ps1` (flat clone into `$SRV_DIR`, default port 8092→8096, port-collision guard, `AF_SEED=1` in generated override), `deploy/DEPLOY.md` (flat path, verified ports table, update-in-place section, pre-deploy port check), `deploy/docker-compose.override.example.yml` (header comment) | manual verification against the live host `ss -tlnp`/`docker ps` (agentfoundry 0.0.0.0:8096->8080, repo flat at /srv/agentfoundry); .ps1 heredoc escaping re-checked. Docs/scripts only — no app code or test change | 0 (docs/ops) |
| R87 | **Enterprise sign-in redesign (SSO options + branded two-panel)** — S120 | web `auth/AuthGate.tsx` (two-panel brand hero + SSO option buttons + honest not-configured/demo notices + `ssoConfig` prop), new pure `ssoOptions()` helper, `auth/auth.css` (two-panel split + hero + SSO button styles + <820px responsive collapse) | `AuthGate.test.tsx` (+7: ssoOptions configured-vs-demo, hero+bullets render, all 3 SSO buttons + demo tags, Microsoft not-configured notice, Microsoft configured redirects to /auth/sso/microsoft/start, Google/SAML demo notices, SSO hidden on register). AuthGate.tsx at 100% all four metrics; 322 web tests total | 7 |
| R88 | **Login self-heals a stale/drifted durable store (no more 500 "Internal error")** — S121 | backend `auth.ts` `login()` routed through new `resolveOrHealUser(cred)` (rebuilds tenant+user from the credential's embedded copy when the live identity is missing; clean `InvalidCredentialsError` when there is no embedded user) + `tenantStatusOf(tenantId)` (missing tenant → undefined, treated as active) so a verified credential never throws an unmapped error | `auth.test.ts` (+3: self-heal rebuilds identity wiped after rehydrate incl. tenantName→id fallback; InvalidCredentials when no embedded user; missing tenant treated as not-suspended). Root cause proven on the live box by curl (fresh register/login 200, rehydrated demo admin 500). auth.ts back to 100% all four; 1159 backend tests; full suite 100% all-files | 3 |
...
- Web App.tsx branch coverage **87.8%** (was 77%; S118 added broken/weak + good-but-imperfect agent tests through the injectable-agent seam). The residual 10 arms are unreachable-by-construction (static coverage-matrix; export-gated failure banners; 3 deliberately-kept defensive guards) — documented per-branch in KNOWN_GAPS §2; the logic behind each is at 100% in the backend suite.
- **Playwright E2E executed on a real browser (14/06):** 42 tests across 3 specs run under Chromium (desktop + Pixel-7); 41 pass, 1 intentional skip. The browser-binary CDN is blocked in the sandbox build env but the suite runs green on a normal-network machine (`npx playwright install chromium && npx playwright test` from `web/`). A stale uppercase assertion on the masthead track-tag was fixed (case-insensitive).
...
- **OIDC: the JWKS/RS256 Entra verifier is built + tested (S117)** — real signature verification proven offline; only the live Entra tenant/client `ENTRA_*` env binding at deploy remains. Live Foundry/GitHub deploy still require external credentials (out of offline-build scope).
